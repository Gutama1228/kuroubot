...
async def ping_cmd(client, message):
    ...
    mention = await get_var(client.me.id, "emoji3")
    cos_mention2 = mention if mention else MENTION
    mention_id = await get_var(client.me.id, "emoji_id3")
    cos_mention = mention_id if mention_id else "6183961455436498818"
    if gua == True:
      _ping = f"""
**<emoji id={cos_ping}>{cos_ping2}</emoji> ᴘᴏɴɢ: `{str(delta_ping).replace('.', ',')}ms`**
**<emoji id={cos_pong}>{cos_pong2}</emoji> ᴜᴘᴛɪᴍᴇ: {uptime}**
**<emoji id={cos_mention}>{cos_mention2}</emoji> ᴍᴇɴᴛɪᴏɴ: {client.me.mention}"""
    elif gua == False:
      _ping = f"""
**{cos_ping2} ᴘᴏɴɢ: `{str(delta_ping).replace('.', ',')} ms`**
**{cos_pong2} ᴜᴘᴛɪᴍᴇ: {uptime}**
**{cos_mention2} ᴍᴇɴᴛɪᴏɴ: {client.me.mention}"""
        ...
        if message.entities:
            for entity in message.entities:
                if entity.custom_emoji_id:
                    ...
                    break
            if emoji_id:
                await set_var(client.me.id, "emoji_id3", emoji_id)
                await jing.edit(
                    f"<b>Emoji 3 Diatur ke: </b> <emoji id={emoji_id}>{emojinya}</emoji>"
                )
    elif gua == False:
        await set_var(client.me.id, "emoji3", emojinya)
        await jing.edit(f"**Kostum emoji 3 diatur ke `{emojinya}`**")


async def set_emoji4(client, message):
    jing = await message.reply("`Processing...`")
